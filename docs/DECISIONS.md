...
### ADR-043 — Technical practice: library grouping + generated scale books
**2026-07-12.** The library gains a **category** (`SongMeta.category`: repertoire | technical, nil =
repertoire so old metadata still decodes). The sidebar groups into **Repertoire** and **Technical
practice** sections (headers shown only once a technical song exists); a per-row menu action moves a
song between groups (so the user's Czerny etudes can live there too).

Seeded on first run (even into an existing library, gated by a `UserDefaults` version flag so it
respects deletion): two **scale books**, generated by `tools/gen_scales.py` — *Major Scales* (12) and
*Minor Scales* (12 keys × natural/harmonic/melodic = 36), each key a grand-staff, both-hands (LH two
octaves below RH), 2-octave up-and-down exercise in difficulty (accidental-count) order, correct key
signature and spelling (e.g. F♯ major's E♯). **Both the MusicXML and the MIDI are generated from the
same note data**, so ingestion fuses them 1:1 (verified by `ScaleBookTests`). Each scale ships as a
**pre-named saved section** (`sections.json`), so drilling "C major" is one tap, and progress reuses
all the existing per-song machinery (best %, last-practised, trouble heatmap, the N-clean-pass drill).

**Rejected:** one scale per library entry (24–48 rows is noise; a book with sections is tidier and the
saved-sections already give per-scale focus); hand-authoring the MusicXML (error-prone spelling —
generate it); a separate top-level "Technical" tab (a library grouping is lighter and lets any étude
